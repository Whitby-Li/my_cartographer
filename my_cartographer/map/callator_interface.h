...
#ifndef MY_CARTOGRAPHER_SENSOR_CALLATOR_INTERFACE_H
#define MY_CARTOGRAPHER_SENSOR_CALLATOR_INTERFACE_H
...
namespace my_cartographer
{
  namespace map
  {
    ...
    public:
      ...
      // Adds a trajectory to produce sorted sensor output for. 
      // Calls 'callback' for each collated sensor data.
      ...
      // 'data' must contain valid sensor data. 
      // Sensor packets with matching 'data.sensor_id_' must be added in time order.
      ...
      // Must only be called if at least one unfinished trajectory exists. 
      // Returns the ID of the trajectory that needs more data before CollatorInterface is unblocked. 
      // Returns 'nullopt' for implementations that do not wait for a particular trajectory.
...
#endif // MY_CARTOGRAPHER_SENSOR_CALLATOR_INTERFACE_H