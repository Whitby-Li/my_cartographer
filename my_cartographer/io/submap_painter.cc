...
#include "my_cartographer/io/submap_painter.h"
...
namespace my_cartographer
{
  namespace io
  {
    ...
    namespace
    {
      ...
      Eigen::Affine3d ToEigen(const my_cartographer::transform::Rigid3d &rigid3)
      {
      ...
      bool Has2DGrid(const map::proto::Submap &submap)
      {
        return submap.has_submap_2d() && submap.submap_2d().has_grid();
      }

      bool Has3DGrids(const map::proto::Submap &submap)
      {
    ...
    {
      my_cartographer::map::proto::SubmapQuery::Response response;
      my_cartographer::transform::Rigid3d local_pose;
      if (proto.has_submap_3d())
      {
        map::Submap3D submap(proto.submap_3d());
        local_pose = submap.local_pose();
        submap.ToResponseProto(global_submap_pose, &response);
      }
      else
      {
        my_cartographer::map::Submap2D submap(proto.submap_2d(),
                                                 conversion_tables);
        local_pose = submap.local_pose();
        submap.ToResponseProto(global_submap_pose, &response);
      }
      ...
      submap_slice->slice_pose =
          my_cartographer::transform::ToRigid3(texture_proto.slice_pose());
    ...
    void DeserializeAndFillSubmapSlices(
        ...
        std::map<map::SubmapId, SubmapSlice> *submap_slices,
        map::ValueConversionTables *conversion_tables)
    {
      std::map<map::SubmapId, transform::Rigid3d> submap_poses;
      for (const auto &trajectory : deserializer->pose_graph().trajectory())
      {
        for (const auto &submap : trajectory.submap())
        {
          submap_poses[map::SubmapId(trajectory.trajectory_id(),
                                         submap.submap_index())] =
              transform::ToRigid3(submap.pose());
        }
      }
      map::proto::SerializedData proto;
      while (deserializer->ReadNextSerializedData(&proto))
      {
            ...
            (Has2DGrid(proto.submap()) || Has3DGrids(proto.submap())))
        {
          ...
          const map::SubmapId id{submap.submap_id().trajectory_id(),
                                            ...
                                            const int width, const int height)
    {
      ...
      std::string cells;
      my_cartographer::common::FastGunzipString(compressed_cells, &cells);
