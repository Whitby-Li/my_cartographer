...
#include "my_cartographer/map/detect_floors.h"
...
namespace my_cartographer
{
  namespace map
  {
    ...
    namespace
    {
      ...
      // A union-find structure for assigning levels to 'spans' in the trajectory implemented as a disjoint-set forest:
      ...
      // Cut the trajectory at jumps in z.
      // A new span is started when the current node's z differs by more than kLevelHeightMeters from the median z values.
      ...
      // but should be folded into the levels before and after enterning it.
                                    ...
                                    const Levels &levels)
      {
        ...
        for (size_t i = 0; i < spans.size(); ++i)
        {
          ...
          // If we have a long piece on this floor already, merge this short piece into it.
          ...
          // Otherwise, add this short piece to the level before and after it. It is likely some intermediate level on stairs.
          size_t index = i - 1;
          if (index < spans.size())
          {
