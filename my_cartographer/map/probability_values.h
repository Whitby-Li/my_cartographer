...
#ifndef MY_CARTOGRAPHER_MAP_PROBABILITY_VALUES_H
#define MY_CARTOGRAPHER_MAP_PROBABILITY_VALUES_H
...
namespace my_cartographer
{
  namespace map
  {
    ...
    // Converts a uint16 (which may or may not have the update marker set) to a probability in the range [kMinProbability, kMaxProbability].
    inline float ValueToProbability(const uint16 value)
    {
      return (*kValueToProbability)[value];
    }

    // Converts a uint16 (which may or may not have the update marker set) to a correspondence cost in the range [kMinCorrespondenceCost, kMaxCorrespondenceCost].
...
#endif // MY_CARTOGRAPHER_MAP_PROBABILITY_VALUES_H
