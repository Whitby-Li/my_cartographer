...
#include "my_cartographer/common/config.h"
#include "my_cartographer/common/configuration_file_resolver.h"
...
namespace my_cartographer
{
  namespace io
  {
    ...
    using map::proto::SerializedData;
        ...
        bool include_unfinished_submaps)
    {
      ...
      const auto options =
          map::CreateMapBuilderOptions(&lua_parameter_dictionary);

      common::ThreadPool thread_pool(1);
      CHECK(!options.use_trajectory_builder_2d());
      // We always use 3D here. 2D submaps do not have histograms.
      map::PoseGraph3D pose_graph(
          options.pose_graph_options(),
          absl::make_unique<map::optimization::OptimizationProblem3D>(
      ...
      // trajectory ids.
      map::proto::PoseGraph pose_graph_proto = deserializer.pose_graph();
      const auto &all_builder_options_proto =
          deserializer.all_trajectory_builder_options();

      std::vector<map::proto::TrajectoryBuilderOptionsWithSensorIds>
      ...
      map::MapById<map::SubmapId, transform::Rigid3d> submap_poses;
      for (const map::proto::Trajectory &trajectory_proto :
           pose_graph_proto.trajectory())
      {
        for (const map::proto::Trajectory::Submap &submap_proto :
             trajectory_proto.submap())
        {
          submap_poses.Insert(map::SubmapId{trajectory_proto.trajectory_id(),
                                                submap_proto.submap_index()},
                              transform::ToRigid3(submap_proto.pose()));
        }
      }

      map::MapById<map::NodeId, transform::Rigid3d> node_poses;
      for (const map::proto::Trajectory &trajectory_proto :
           pose_graph_proto.trajectory())
      {
        for (const map::proto::Trajectory::Node &node_proto :
             trajectory_proto.node())
        {
          node_poses.Insert(map::NodeId{trajectory_proto.trajectory_id(),
      ...
      while (deserializer.ReadNextSerializedData(&proto))
      {
        ...
        case SerializedData::kSubmap:
        {
          ...
          submap_id_to_submap.Insert(
              map::SubmapId{proto.submap().submap_id().trajectory_id(),
        ...
        case SerializedData::kNode:
        {
          ...
          const map::NodeId node_id(proto.node().node_id().trajectory_id(),
      ...
      pose_graph.AddSerializedConstraints(
          map::FromProto(pose_graph_proto.constraint()));
      CHECK(input->eof());
